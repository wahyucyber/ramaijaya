		...
		if ($query->num_rows() > 0) {
			...
			foreach ($query->result_array() as $key) {
				...
				$result['Data'][$no++] = [
					...
					'harga' => $key['harga'],
					'diskon' => $key['diskon'],
					'harga_diskon' => (strtotime($key['diskon_dari']) >= strtotime(date('Y-m-d')) && strtotime($key['diskon_ke']) <= strtotime(date('Y-m-d'))) ? ceil($key['harga'] - (($key['diskon']/100)*$key['harga'])) : 0,
					'berat' => $key['berat'],
			...
			if ($st == "produk" || $st == "katalog") {
				...
				foreach ($get_produk->result_array() as $key) {
					$hasil['Data'][$no++] = array(
						...
						'keterangan' => $key['keterangan'],
						'harga' => $key['harga'],
						'diskon' => (strtotime($key['diskon_dari']) >= strtotime(date('Y-m-d')) && strtotime($key['diskon_ke']) <= strtotime(date('Y-m-d'))) ? $key['diskon'] : 0,
						'harga_diskon' => (strtotime($key['diskon_dari']) >= strtotime(date('Y-m-d')) && strtotime($key['diskon_ke']) <= strtotime(date('Y-m-d'))) ? ceil($key['harga'] - (($key['diskon']/100)*$key['harga'])) : 0,
						'berat' => $key['berat'],
						'stok' => $key['stok'],
						// 'favorit' => $key['favorit'],
